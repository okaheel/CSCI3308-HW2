...
function begin_play() {
	...
	this.started = true;
	document.getElementById("turn_info").value = "Turn: " + player1.value
	document.getElementById("turn_info").innerText = "Turn: " + player1.value
}
...
function play() {
	...
	if (isEmpty(moveTextField.value)) {
		alert("Move can't be empty");
		return
	}


	var moveRegex = /[A-C][1-3]/g;
	var moveValue = moveTextField.value.toUpperCase();
	var validMove = moveTextField.value.toUpperCase().match(moveRegex);

	if (!validMove) {
		alert("Invalid Move");
		return
	}
	if (document.getElementById(moveValue).innerText == "X" || document.getElementById(moveValue).innerText == "O") {
		alert("Invalid Move")
		return
	}
	if (turn == true) {
	...
	switch (moveValue) {
		case "A1":
			board_state[0] = valueChecker
			break
		case "A2":
			board_state[1] = valueChecker
			break
		case "A3":
			board_state[2] = valueChecker
			break
		case "B1":
			board_state[3] = valueChecker
			break
		case "B2":
			board_state[4] = valueChecker
			break
		case "B3":
			board_state[5] = valueChecker
			break
		case "C1":
			board_state[6] = valueChecker
			break
		case "C2":
			board_state[7] = valueChecker
			break
		case "C3":
			board_state[8] = valueChecker
			break
		default:
			break;
	}
	if (turn == 1) {
		document.getElementById(moveValue).innerText = "X";
	} else if (turn == 0) {
		document.getElementById(moveValue).innerText = "O";
	}
...
function checkWinner() {
	...
	if ((board_state[0] == valueChecker) && (board_state[1] == valueChecker) && (board_state[2] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[0] == valueChecker) && (board_state[4] == valueChecker) && (board_state[8] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[2] == valueChecker) && (board_state[5] == valueChecker) && (board_state[8] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[0] == valueChecker) && (board_state[3] == valueChecker) && (board_state[6] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[1] == valueChecker) && (board_state[4] == valueChecker) && (board_state[7] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[2] == valueChecker) && (board_state[4] == valueChecker) && (board_state[6] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[3] == valueChecker) && (board_state[4] == valueChecker) && (board_state[5] == valueChecker)) {
		callWinner(valueChecker);
	} else if ((board_state[6] == valueChecker) && (board_state[7] == valueChecker) && (board_state[8] == valueChecker)) {
		callWinner(valueChecker);
	}
}
function callWinner(turnNumber) {
	if (turnNumber == 1) {
		alert("Winner!: X")
	} else {
		alert("Winner!: O")
